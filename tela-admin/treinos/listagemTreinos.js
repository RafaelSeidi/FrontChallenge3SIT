...
function construirCard(json){
    ...
    for (var i = 0; i < json.length; i++) {
        var row = `<div class="card-treino">
            ...
            <div class="box-btn">
                <a href="" class="btn-pausar">Pausar</a>
                <a href="" class="btn-excluir" onclick="removerTreinamento(${json[i].treinamentoId})">Excluir</a>
            </div>
        </div>`
        div.innerHTML += row;
    }
...
function removerTreinamento(treinamentoId){
    ...
    if (confirm(`Você realmente deseja remover o treinamento com ID ${treinamentoId}?`)) {
        const myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");

        const raw = JSON.stringify(treinamentoId);
        ...
        fetch("http://localhost:8094/treinamento/remover", requestOptions)
            .then((response) => response.json())
            .then((json) => {
                if (json.length !== 0) {
                    console.log(json);
